...
function sortStandards(standardsArray) {
    return standardsArray.sort((a, b) => {
        const indexA = AVAILABLE_STANDARDS.indexOf(a);
        const indexB = AVAILABLE_STANDARDS.indexOf(b);
    ...
    container.innerHTML = `<div class="chip-grid">` + AVAILABLE_STANDARDS.map((std, index) => {
        ...
        <div class="std-selection-block ${isSelected ? 'active-block' : ''}">
            <div class="std-chip ${isSelected ? 'active' : ''}" onclick="toggleStandard('${std}', ${index})">
                ${std + (std.startsWith("NTF 4073:") ? " (HACCP)" : "")}
            </div>
            ${detailsPanel}
        </div>
        `;
...
function isProcessesComplete() {
    ...
    return appState.processes.every(proc =>
        proc.name &&
        proc.name.trim() !== "" &&
        !/^Proceso \d+$/.test(proc.name.trim()) && // Reject default names
...
function renderProcesses() {
    ...
    appState.processes.forEach((proc, index) => {
        ...
        if (proc.id !== expandedProcessId) {
            ...
            let chipsHtml = sortStandards(Array.from(mappedSchemes))
            ...
            if (activeStandards.length === 0) {
                ...
                // Ensure an active standard is selected for this specific process
                if (!activeProcessScheme[proc.id] || !activeStandards.includes(activeProcessScheme[proc.id])) {
                ...
                const stripHtml = `<div class="scheme-strip" style="margin-top: 0; padding-bottom: 10px; border-bottom: 1px solid #eee;">` 
                    + activeStandards.map(std => `
                        <div class="scheme-tab ${std === activeStd ? 'active' : ''}" onclick="switchProcessScheme(${proc.id}, '${std}')">
                            ${std}
                        </div>
                    `).join('') + `</div>`;

                // Build clauses ONLY for the active standard
                let clausesHtml = "";
                const chapters = Object.keys(grouped[activeStd]).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
                chapters.forEach(chap => {
                    const clauses = Array.from(grouped[activeStd][chap]).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
                    clausesHtml += `
                        <div class="na-chapter-group" style="margin-top: 15px;">
                ...
                detailsHtml = stripHtml + clausesHtml;
            }
            ...
            card.innerHTML = `
                <div class="pw-expanded" onclick="toggleProcessExpand(${proc.id}, event)">
                    ...
                    <div class="pw-expanded-footer" style="display: flex; justify-content: flex-end; align-items: center; margin-top: 5px; padding-top: 5px;">
                        <button class="pw-action-btn delete" onclick="deleteProcessUI(${proc.id})" title="Eliminar Proceso">🗑️ Eliminar</button>
                        </div>
                </div>
            `;
...
function renderProcessesStatus() {
    ...
    const incomplete = appState.processes.filter(proc => !proc.assignedRequirements || proc.assignedRequirements.length === 0);
    const invalidNames = appState.processes.filter(p => /^Proceso \d+$/.test(p.name.trim()));

    if (incomplete.length > 0 || invalidNames.length > 0) {
        let html = "";
        ...
        if (invalidNames.length > 0) {
            html += `
                <div class="missing-title" style="margin-top: 10px; color: #856404;">Nombres por defecto detectados</div>
                ${invalidNames.map(p => `<div class="missing-std-block" style="color: #664d03;">• Debes asignar un nombre real a "${p.name}".</div>`).join('')}
            `;
        }
...
function renderExportStatus() {
    ...
    if (!appState.processes || appState.processes.length === 0) {
        ...
        if (incomplete.length > 0) messages.push("Existen procesos sin requisitos asignados.");
        if (invalidNames.length > 0) messages.push("Existen procesos con nombres por defecto. Asígnales un nombre real.");
        if (!isUnderProcessLimit) messages.push(`Excediste el límite de ${PDF_MAX_PROCESSES} procesos requeridos por el estándar PDF.`);
    }
...
function clearAllNa() {
    if(confirm("¿Está seguro de eliminar todas las exclusiones definidas?")) {
        appState.nonApplicableClauses = [];
        appState.nonApplicableIntent = [];
        appState.naLastPath = [];
        saveAppState();
...
function renderNaSummary() {
    ...
    let btnClear = document.getElementById("btn-clear-na");
    ...
    // 1. Logic for dynamic "Limpiar" button
    if (!appState.nonApplicableClauses || appState.nonApplicableClauses.length === 0) {
        schemeStrip.classList.add("hidden");
        statusBadge.className = "badge-status empty";
        statusBadge.textContent = "Sin exclusiones";
        ...
        `;
        if (btnClear) btnClear.classList.add("hidden"); // Hide clear button if empty
        return;
    }

    // 2. Complete State Rules
    ...
    // Inject or show the clear button
    if (btnManage) {
        if (!btnClear) {
            btnClear = document.createElement("button");
            btnClear.id = "btn-clear-na";
            btnClear.className = "ux-btn";
            btnClear.style.marginLeft = "10px";
            btnClear.style.backgroundColor = "#dc3545"; // Red danger color
            btnClear.style.color = "white";
            btnClear.textContent = "Limpiar";
            btnClear.onclick = clearAllNa;
            btnManage.parentNode.appendChild(btnClear);
        }
...
function toggleProcessExpand(id, event) {
    ...
    if (event && event.target.closest('input')) return; // Ignore input clicks
    if (event && event.target.closest('.scheme-strip')) return; // Ignore tab clicks!
...
function switchProcessScheme(procId, std) {
    activeProcessScheme[procId] = std;
    renderProcesses();
}
